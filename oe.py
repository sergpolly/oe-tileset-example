...
def get_data(
    ...
    # chrom_enum look up table ... i.e. 0 -> chr1, 1 -> chr2, etc
    chrom_enum = pd.DataFrame({"chrom": chroms}).reset_index().rename(columns={"index":"chrom_enum"})
    # replace chroms in view_df with chrom_enums, and rename them back to chroms ...
    view_df = view_df.merge(
            chrom_enum,
            on="chrom",
            how="left"
        )[["chrom_enum","start","end","name"]].rename(columns={"chrom_enum":"chrom"})
    # dataframe to numpy array for "assign_supports" to work well:
    ...
    bins["r"] = assign_supports(bins, view_array)
    ...
    pixels = pd.merge(
        pixels,
        expected_df[["r1","r2","dist","expected"]],
        how="left",
        on=["r1","r2","dist"],
    )
    ...
    bins1 = bins[i0 : i1 + 1].copy()
    bins2 = bins[j0 : j1 + 1].copy()
    ...
    if (transform == "default" and "weight" in c.bins()) or transform == "weight":
        ...
        pixels["balanced"] = pixels["balanced"] / pixels["expected"]
        # pixels[oe_column_name] = pixels[observed_column_name] / pixels[expected_column_name]
        pixels = pixels[["genome_start1", "genome_start2", "balanced"]]

        return (pixels, (bins1, bins2))
    elif transform in ("KR", "VC", "VC_SQRT"):
        pixels["balanced"] = (
...
def tiles(filepath, expected, view, tile_ids):
    """ """
    ...
    generated_tiles = generate_tiles(filepath, expected, view, new_tile_ids)
...
def generate_tiles(filepath, expected, view, tile_ids):
    """
    ...
    partitioned_tile_ids = list(
        it.chain(
            *[
                hgut.partition_by_adjacent_tiles(t)
    ...
    for tile_group in partitioned_tile_ids:
        ...
        tile_positions = list(
            filter(lambda x: x[0] < tileset_info["max_pos"][0] + 1, tile_positions)
        )
        tile_positions = list(
            filter(lambda x: x[1] < tileset_info["max_pos"][1] + 1, tile_positions)
        )
        ...
        tile_data_by_position = make_tiles(
            ...
            maxy - miny + 1,
            # new params
            expected_df=expected[resolution],
            view_df=view,
        )
        ...
        tiles = [
            (
                ".".join(
                    map(
                        str,
                        [tileset_id] + [zoom_level] + list(position) + [transform_type],
                    )
                ),
